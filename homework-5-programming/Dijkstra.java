...
public class Dijkstra {
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
	  double temp =  Math.pow(ux-vx,2) + Math.pow(uy-vy,2);
	  temp =  Math.sqrt(temp);
	  return temp;
  }
  ...
  public void computeAllEuclideanDistances() {
        for(Vertex ver : getVertices()){ //could also use keySet()
			for(Edge edg : ver.adjacentEdges){
				Vertex v = edg.target;
				edg.distance = computeEuclideanDistance(ver.x, ver.y, v.x, v.y);
			}
		}
  }
  ...
  public void doDijkstra(String s) {
	  ...
	  Vertex u = vertexNames.get(s); //source
	  u.distance = 0.0;
	  u.known = true;
	  vertexList.add(u);
	  
	  while(vertexList.size() > 0){
		  Vertex min = vertexList.getFirst();
		  for(Vertex temp : vertexList){
			  if(temp.distance < min.distance){
				  min = temp;
			  }
		  }
		  vertexList.remove(min);
		  ...
		  for(Edge edg : u.adjacentEdges){
			  vertex = edg.target;
			  if(vertex.known == false && (u.distance + edg.distance < vertex.distance)){
				  vertex.prev = u;
				  vertex.distance = edg.distance + u.distance;
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
	  ...
	  Vertex ver = vertexNames.get(t);
	  
	  //use the prev variable to go backwards from t
	  while(ver.prev != null){
		  result.addFirst(new Edge(ver.prev, ver, computeEuclideanDistance(ver.prev.x, ver.prev.y, ver.x, ver.y)));
		  ver = ver.prev;
	  }
